...
cdef class RegionGrow3D:
    ...
    def __cinit__(self, int[:,:,:] slices, int[:,:,:] masks, 
                  int upperThreshold, int lowerThreshold, neighborMode):
        self.slices = slices
        self.masks = masks
        self.tracheaMask = np.zeros_like(self.slices)
    ...
    def main(self, seed):
        ...
        self.tracheaMask[newItem[0], newItem[1], newItem[2]] = 1
        ...
        while len(self.queue) != 0:
            ...
            if self.neighborMode == "26n":
                ...
                self.checkNeighbour(newItem[0]+1, newItem[1], newItem[2])
        return self.tracheaMask
        
    cdef checkNeighbour(self, int z, int y, int x):
        cdef int intensity
        if x < 513 and y < 513 and z < 513 and x > -1 and y > -1 and z > -1 and self.masks[z,y,x] == 1:
            intensity = self.slices[z, y, x]
            if self.isIntensityAcceptable(intensity) and self.tracheaMask[z,y,x] == 0:
                self.tracheaMask[z,y,x] = 1
